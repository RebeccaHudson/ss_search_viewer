...
class SearchBySnpidTests(ScoresViewerTestCase):
    ...
    def test_that_basic_snpid_search_can_be_paged(self):
        url = reverse('ss_viewer:snpid-search')
        base_request =  { 'raw_requested_snpids' : 'rs202068986 rs371194064  rs202068986' +\
                                                   'rs199706086 rs111200574',
                          'pvalue_rank_cutoff'   : 1,
                          'action'               : self.btn_action }
        self.page_through_request_data( url, base_request, 'snpid_search_form')
    ...
    def test_that_scores_list_loads_post_for_no_matching_snpids(self):
      response = self.client.post(reverse('ss_viewer:snpid-search'),
             { 'raw_requested_snpids'  : 'rs1111111, rs11111111111111, rs1',
    ...
    def test_page_through_snpid_file_search_response_data(self):
         response = None
         url = reverse('ss_viewer:snpid-search')
         fpath = os.path.dirname(os.path.abspath(__file__))+'/tinysnp.txt'
         fp = open(fpath, 'r')
         base_request = {'pvalue_rank_cutoff' :0.05,
                         'file_of_snpids'     :fp,
                         'action'             : self.btn_action }
         response = self.client.post(url, base_request )

         snpids_scraped_out_of_form = \
         ...
         while response.context['search_paging_info']['show_next_btn'] is True:
              ...
              next_page_request = \
                         {'pvalue_rank_cutoff'   :  0.05,
                         'raw_requested_snpids'  :  snpids_scraped_out_of_form, 
                         'action'                : 'Next',
                         'page_of_results_shown' :  page_shown }
